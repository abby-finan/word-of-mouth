...
export default function FriendsPage() {
  ...
  async function handleAddFriend(e: React.FormEvent) {
    ...
    if (error) {
      setAddError(error.includes("duplicate") ? "Friend request already sent." : error);
    } else {
      setAddSuccess(`Friend request sent to ${user.first_name}!`);
      setEmail("");
    }
    setAdding(false);
  }
        ...
        {friends.length === 0 ? (
          <div className="text-center py-12 px-4">
            <p className="text-warm-gray text-sm leading-relaxed">
              Add friends to see their trusted service providers. Share your
              recommendations to unlock theirs.
            </p>
          </div>
        ) : (
          <div className="space-y-2">
